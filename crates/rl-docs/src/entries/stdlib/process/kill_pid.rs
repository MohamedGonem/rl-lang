use crate::entry::FnEntry;

pub static KILL_PID: FnEntry = FnEntry {
    signature: "kill_pid(pid)",
    description: "sends SIGKILL to the given process for immediate termination",
    example: r#"get std::process::exec_background
get std::process::kill_pid

dec int pid = exec_background("sleep 60")?
kill_pid(pid)?"#,
    expected_output: None,
    returns: "result[null]",
    errors: Some("Will return error if the signal could not be sent"),
    see_also: &["term_pid", "exec_background"],
    since: Some("v2.1.0"),
};
