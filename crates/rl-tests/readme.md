...
Part of the [rl-lang](https://github.com/rl-lang/rl-lang) workspace. This crate is not published (`publish = false`) - it exists purely to hold end-to-end tests that exercise the lexer, parser, checker, and VM together.
...
| `tests/checker` | Type-checking |
| `tests/common` | Shared test fixtures and helpers |

## Dependencies

Depends (as dev-dependencies) on `rl-ast`, `rl-lexer`, `rl-parser`, `rl-resolver`, `rl-utils`, `rl-checker`, `rl-tooling`, and `rl-vm`.
