...
impl<'a> CCodegen<'a> {
    ...
    pub fn compile_func_call(&mut self, path: &[String], args: &[ExprId]) -> Result<(), Error> {
        ...
        match func_name {
            ...
            | "sqrt" | "exp" | "log2" | "log10" | "ceil" | "floor" | "round" => {
            ...
            "abs" => {
                self.writer.write("(int64_t)llabs(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "count_bits" => {
                self.writer.write("__builtin_popcountll(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "leading_zeros" => {
                self.writer.write("__builtin_clzll(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "trailing_zeros" => {
                self.writer.write("__builtin_ctzll(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "slice" => {
                self.writer.write("rl_str_slice(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(", ");
            ...
            "char_at" => {
                self.writer.write("rl_str_char_at(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            // ---- fs ----
            "file_size" => {
                self.writer.write("rl_fs_file_size(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "file_modified" => {
                self.writer.write("rl_fs_file_modified(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "copy_file" => {
                self.writer.write("rl_fs_copy_file(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "mkdir_all" => {
                self.writer.write("rl_fs_mkdir_all(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "rmdir_all" => {
                self.writer.write("rl_fs_rmdir_all(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "list_dir" => {
                self.writer.write("rl_fs_list_dir(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            // ---- process ----
            "cwd" => {
                self.writer.write("rl_process_cwd()");
                return Ok(());
            }
            "set_cwd" => {
                self.writer.write("rl_process_set_cwd(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "exec" => {
                self.writer.write("rl_process_exec(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "exec_code" => {
                self.writer.write("rl_process_exec_code(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "exec_lines" => {
                self.writer.write("rl_process_exec_lines(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "with_exec" => {
                self.writer.write("rl_process_with_exec(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "with_exec_code" => {
                self.writer.write("rl_process_with_exec_code(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "with_exec_lines" => {
                self.writer.write("rl_process_with_exec_lines(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            // ---- time (extended) ----
            "format_time" => {
                self.writer.write("rl_time_format_time(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "format_date_str" => {
                self.writer.write("rl_time_format_date_str(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "format_time_str" => {
                self.writer.write("rl_time_format_time_str(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "time_parts" => {
                self.writer.write("rl_time_parts(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "read" => {
                self.writer.write("rl_io_read()");
                return Ok(());
            }
            "read_int" => {
                self.writer.write("rl_io_read_int()");
                return Ok(());
            }
            "read_float" => {
                self.writer.write("rl_io_read_float()");
                return Ok(());
            }
            "write_file" => {
            ...
            "delete_file" => {
                self.writer.write("rl_io_delete_file(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            // ---- types ----
            "to_string" => {
                self.writer.write("rl_types_to_string(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "to_bin" => {
                self.writer.write("rl_types_to_bin(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "to_hex" => {
                self.writer.write("rl_types_to_hex(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "to_oct" => {
                self.writer.write("rl_types_to_oct(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "to_int" => {
                self.writer.write("(int64_t)");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                return Ok(());
            }
            "to_float" => {
            ...
            "rand_int_range" => {
                self.writer.write("rl_rand_int_range(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "rand_float_range" => {
                self.writer.write("rl_rand_float_range(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "rand_dice" => {
                self.writer.write("rl_rand_dice(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "rand_range" => {
                self.writer.write("rl_rand_range(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "rand_range_step" => {
                self.writer.write("rl_rand_range_step(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(", ");
            ...
            "rand_string" => {
                self.writer.write("rl_rand_string(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "set_remove" => {
                self.writer.write("rl_set_remove_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "set_contains" => {
                self.writer.write("rl_set_contains_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "set_to_array" => {
                self.writer.write("rl_set_to_array(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "map_remove" => {
                self.writer.write("rl_map_remove_val(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "map_keys" => {
                self.writer.write("rl_map_keys(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "map_values" => {
                self.writer.write("rl_map_values(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "map_to_array" => {
                self.writer.write("rl_map_to_array_s(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            ...
            "arr_first" => {
                self.writer.write("rl_arr_first_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_last" => {
                self.writer.write("rl_arr_last_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_contains" => {
                self.writer.write("rl_arr_contains_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "arr_index_of" => {
                self.writer.write("rl_arr_index_of_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "arr_reverse" => {
                self.writer.write("rl_arr_reverse_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_concat" => {
                self.writer.write("rl_arr_concat_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "arr_unique" => {
                self.writer.write("rl_arr_unique_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_slice" => {
                self.writer.write("rl_arr_slice_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(", ");
            ...
            "arr_fill" => {
                self.writer.write("rl_arr_fill_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "arr_range" => {
                self.writer.write("rl_arr_range_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(", ");
            ...
            "arr_sum" => {
                self.writer.write("rl_arr_sum_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_product" => {
                self.writer.write("rl_arr_product_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_max" => {
                self.writer.write("rl_arr_max_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_min" => {
                self.writer.write("rl_arr_min_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_sort" => {
                self.writer.write("rl_arr_sort_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_flatten" => {
                self.writer.write("rl_arr_flatten_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_push" => {
                self.writer.write("rl_arr_push_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
            ...
            "arr_pop" => {
                self.writer.write("rl_arr_pop_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(")");
                return Ok(());
            }
            "arr_insert" => {
                self.writer.write("rl_arr_insert_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(", ");
            ...
            "arr_remove" => {
                self.writer.write("rl_arr_remove_i64(");
                if !args.is_empty() { self.compile_expr(args[0])?; }
                self.writer.write(", ");
                if args.len() >= 2 { self.compile_expr(args[1])?; }
                self.writer.write(")");
