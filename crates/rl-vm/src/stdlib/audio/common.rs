...
pub fn extract_float(value: VmValue, name: &str) -> Result<f64, String> {
    match value {
...
pub fn open_stream(vm: &Vm) -> Result<rodio::MixerDeviceSink, String> {
    let mut stream = match &vm.audio_output_device {
        Some(name) => {
...
pub fn new_sink(vm: &Vm) -> Result<(rodio::Player, rodio::MixerDeviceSink), String> {
    let stream = open_stream(vm)?;
    let sink = rodio::Player::connect_new(stream.mixer());
    sink.set_volume(vm.audio_master_volume);
    Ok((sink, stream))
}
