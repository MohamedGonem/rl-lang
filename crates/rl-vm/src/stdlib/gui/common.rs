use crate::{Vm, stdlib::gui::GuiHandle, values::VmValue};
use rl_ast::statements::HandleKind;
use rl_utils::{errors::Error, span::Span};

pub fn insert_handle(eval: &mut Vm, handle: GuiHandle) -> VmValue {
    let id = eval.gui_next_handle;
    eval.gui_next_handle += 1;
    eval.gui_handles.insert(id, handle);
    VmValue::Handle {
        kind: HandleKind::Gui,
        id,
    }
}

pub fn attach_child(eval: &mut Vm, window_id: u64, child_id: u64) {
    if let Some(GuiHandle::Window(w)) = eval.gui_handles.get_mut(&window_id) {
        w.children.push(child_id);
    }
}

pub fn require_window(eval: &Vm, window_id: u64, fn_name: &str) -> Result<(), String> {
    match eval.gui_handles.get(&window_id) {
        Some(GuiHandle::Window(_)) => Ok(()),
        Some(_) => Err(format!("{}: handle {} is not a window", fn_name, window_id)),
        None => Err(format!("{}: unknown handle {}", fn_name, window_id)),
    }
}

/// Reports a callback error the same way top-level rl-lang errors are
/// reported, instead of silently discarding it.
pub fn report_callback_err(result: Result<VmValue, Error>) {
    if let Err(e) = result {
        e.report_to_stderr();
    }
}

/// Removes a window handle and all of its child widget handles, firing the
/// window's `on_close` callback (if any) first. Used both by `gui_close` and
/// by `gui_run` when a window's native close button is clicked. No-op if
/// `id` isn't a window (or doesn't exist).
pub fn close_window(eval: &mut Vm, id: u64) {
    let Some(GuiHandle::Window(w)) = eval.gui_handles.remove(&id) else {
        return;
    };
    for child in &w.children {
        eval.gui_handles.remove(child);
    }
    if let Some(cb) = w.on_close {
        report_callback_err(eval.call_value(&cb, &[], Span::dummy()));
    }
}

/// Extracts `array[string]` into a `Vec<String>`, for `gui_dropdown`/`gui_radio_group`.
pub fn extract_string_array(value: VmValue, name: &str) -> Result<Vec<String>, String> {
    match value {
        VmValue::Arr(items) => items
            .iter()
            .cloned()
            .map(|v| crate::stdlib::common::extract_string(v, name))
            .collect(),
        other => Err(format!(
            "{}: expected array[string], got {}",
            name,
            other.type_name()
        )),
    }
}

/// Extracts `array[int]` into a `Vec<u8>`, validating every element is a byte
/// (0-255). Used by `gui_window_set_icon` for raw RGBA pixel data.
pub fn extract_byte_array(value: VmValue, name: &str) -> Result<Vec<u8>, String> {
    match value {
        VmValue::Arr(items) => items
            .iter()
            .cloned()
            .map(|v| {
                let n = crate::stdlib::common::extract_int(v, name)?;
                if !(0..=255).contains(&n) {
                    return Err(format!(
                        "{}: byte value {} is out of range (must be 0-255)",
                        name, n
                    ));
                }
                Ok(n as u8)
            })
            .collect(),
        other => Err(format!(
            "{}: expected array[int], got {}",
            name,
            other.type_name()
        )),
    }
}
