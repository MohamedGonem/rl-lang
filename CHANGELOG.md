...
- **C transpiler (`rl-cc`)** - transpiles rl to C99. Full pipeline works end-to-end: lex, parse, resolve, type-check, C codegen, optional `cc` compilation. Supported features: types, arithmetic, booleans, comparisons, control flow (if/else, while, for, foreach, forrange, loop, break, continue), functions, casts, println/print, tuples, tuple destruction, arrays, maps, sets, records/structs, enums/tags, match, result type (ok/err/error), error propagation (`?`), impl methods, escape sequences, closures, lambda expressions, multi-file imports (resolved at resolve time, inlined into generated C). Runtime includes `rl_string`, `rl_result` (tagged union with type tag enum), `rl_value` (tagged union for map/set storage), `rl_array`, `rl_map`, `rl_set`, `rl_closure`, and per-program record/tuple/enum print functions. CLI: `rl transpile file.rl --compile`.
